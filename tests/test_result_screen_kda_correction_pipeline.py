...
class TestResultScreenKdaCorrectionPipeline(unittest.TestCase):
    ...
    def test_pipeline_skips_sessions_without_result_dump(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            memory_sessions = root / "memory_sessions"
            empty_session = memory_sessions / "empty_probe"
        ...
        self.assertEqual(report["discovered_session_count"], 1)
        self.assertEqual(report["failed_session_count"], 0)
