"""Compare one credit action family against same-hero peers for minion outliers."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Dict, List, Optional

from .minion_hero_compare import build_minion_hero_compare


def build_minion_action_value_compare(
    truth_path: str,
    target_replay_name: str,
    action_hex: str = "0x02",
    top_pattern_limit: int = 16,
) -> Dict[str, object]:
    """Compare one action family's value buckets against same-hero peers."""
    compare = build_minion_hero_compare(
        truth_path=truth_path,
        target_replay_name=target_replay_name,
        top_pattern_limit=top_pattern_limit,
    )

    selected_patterns = [pattern for pattern in compare["selected_patterns"] if pattern.startswith(action_hex.lower()) or pattern.startswith(action_hex.upper())]
    rows = []
    for row in compare["rows"]:
        value_scores = []
        for pattern in selected_patterns:
            target_count = row["target_pattern_counts"].get(pattern, 0)
            peer_values = [peer["pattern_counts"].get(pattern, 0) for peer in row["same_hero_peers"]]
            if peer_values:
                peer_mean = sum(peer_values) / len(peer_values)
                peer_max = max(peer_values)
                nonzero_peer_count = sum(1 for value in peer_values if value > 0)
                excess_vs_mean = target_count - peer_mean
                excess_vs_max = target_count - peer_max
            else:
                peer_mean = None
                peer_max = None
                nonzero_peer_count = 0
                excess_vs_mean = None
                excess_vs_max = None
            value_scores.append(
                {
                    "pattern": pattern,
                    "target_count": target_count,
                    "peer_mean": peer_mean,
                    "peer_max": peer_max,
                    "peer_count": len(peer_values),
                    "nonzero_peer_count": nonzero_peer_count,
                    "excess_vs_mean": excess_vs_mean,
                    "excess_vs_max": excess_vs_max,
                }
            )
        value_scores.sort(
            key=lambda item: (
                item["excess_vs_mean"] if item["excess_vs_mean"] is not None else float("-inf"),
                item["target_count"],
            ),
            reverse=True,
        )
        rows.append(
            {
                "player_name": row["player_name"],
                "hero_name": row["hero_name"],
                "residual_vs_0e": row["residual_vs_0e"],
                "same_hero_peer_count": row["same_hero_peer_count"],
                "value_scores": value_scores,
            }
        )

    return {
        "truth_path": str(Path(truth_path).resolve()),
        "target_replay_name": target_replay_name,
        "action_hex": action_hex,
        "selected_patterns": selected_patterns,
        "rows": rows,
    }


def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Compare one credit action family against same-hero peers for minion outliers.")
    parser.add_argument("--truth", default="vg/output/tournament_truth.json", help="Truth JSON path")
    parser.add_argument("--replay-name", required=True, help="Target replay_name from truth JSON")
    parser.add_argument("--action", default="0x02", help="Credit action family to inspect")
    parser.add_argument("--top-pattern-limit", type=int, default=16, help="How many patterns to include")
    parser.add_argument("-o", "--output", help="Optional output JSON path")
    args = parser.parse_args(argv)

    report = build_minion_action_value_compare(
        truth_path=args.truth,
        target_replay_name=args.replay_name,
        action_hex=args.action,
        top_pattern_limit=args.top_pattern_limit,
    )
    payload = json.dumps(report, indent=2, ensure_ascii=False)
    if args.output:
        output_path = Path(args.output)
        output_path.write_text(payload, encoding="utf-8")
        print(f"Minion action value compare saved to {output_path}")
    else:
        print(payload)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
