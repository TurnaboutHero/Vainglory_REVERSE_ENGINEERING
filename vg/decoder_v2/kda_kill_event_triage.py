"""Inspect raw kill-event timelines for player-level kill mismatches."""
...
def build_kda_kill_event_triage(
    truth_path: str,
    *,
    kill_buffer: int = 20,
    death_buffer: int = 8,
    ...
    kill_rows = [row for row in mismatch_report["rows"] if row["diff"]["kills"] != 0]

    replay_cache: Dict[str, Dict[str, object]] = {}
    triage_rows = []

    for mismatch in kill_rows:
        ...
        pairs = detector.get_kill_death_pairs(team_map, game_duration=duration, death_buffer=death_buffer)

        events = []
        for kill_event in detector.kill_events:
            if kill_event.killer_eid != player_eid:
                continue
            pair = next(
                (
                    row
                    for row in pairs
                    if row["killer_eid"] == player_eid
                    and row["kill_ts"] is not None
                    and kill_event.timestamp is not None
                    and abs(row["kill_ts"] - kill_event.timestamp) < 1e-6
                ),
                None,
            )
            victim_name = player_map.get(pair["victim_eid"], {}).get("name") if pair and pair["victim_eid"] else None
            events.append(
                {
                    "timestamp": kill_event.timestamp,
                    "frame_idx": kill_event.frame_idx,
                    "file_offset": kill_event.file_offset,
                    "delta_after_duration": round((kill_event.timestamp or 0) - duration, 3) if kill_event.timestamp is not None else None,
                    "victim_name": victim_name,
                    "pair_dt": pair["dt"] if pair else None,
                    "credit_values": [(credit.eid, credit.value) for credit in kill_event.credits],
                }
            )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Inspect raw kill-event timelines for player kill mismatches.")
    ...
    report = build_kda_kill_event_triage(
        args.truth,
    ...
    if args.output:
        ...
        print(f"KDA kill event triage saved to {args.output}")
    else:
        print(payload)
    return 0
