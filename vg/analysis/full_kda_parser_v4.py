"""
Full KDA Parser v4 - Positional scan with kill credit block parsing
===================================================================
The kill credit block has a specific structure:
  1. [victim_eid](1.0) [victim_eid](0.0)   - victim death marker
  2. [killer_eid](large gold) [killer_eid](1.0) - kill bounty
  3. For each assister: [eid](gold) [eid](1.0) [eid](0.5 or 1.0)
  4. Bonus/XP: small values for participants

Strategy: scan forward from kill header, find the killer's large gold credit,
then count unique teammates that appear AFTER it with medium gold values.
"""
import os
import struct
import json
from collections import Counter, defaultdict

CACHE_DIR = "D:/Desktop/My Folder/Game/VG/vg replay/21.11.04/cache"
PREFIX = "8fc12404-6151-11eb-afe2-061b3d1d141d-9de666b6-299f-4503-bf4b-4c7b351847f4"

entities_be = {
    0x05DC: "Phinn", 0x05DD: "Yates", 0x05DE: "Caine",
    0x05DF: "Petal", 0x05E0: "Karas", 0x05E1: "Baron",
}

left_team = {"Phinn", "Petal", "Baron"}
right_team = {"Yates", "Caine", "Karas"}

truth = {
    "Phinn": {"K": 2, "D": 0, "A": 6},
    "Yates": {"K": 1, "D": 4, "A": 4},
    "Caine": {"K": 3, "D": 4, "A": 3},
    "Karas": {"K": 0, "D": 3, "A": 2},
    "Petal": {"K": 3, "D": 2, "A": 3},
    "Baron": {"K": 6, "D": 2, "A": 6},
}

print("Loading frames...")
frames = {}
for i in range(200):
    path = os.path.join(CACHE_DIR, f"{PREFIX}.{i}.vgr")
    if not os.path.exists(path):
        continue
    with open(path, "rb") as f:
        frames[i] = f.read()
print(f"Loaded {len(frames)} frames")
print()

KILL_HDR = bytes([0x18, 0x04, 0x1C])
DEATH_HDR = bytes([0x08, 0x04, 0x31])
CREDIT_HDR = bytes([0x10, 0x04, 0x1D])

def get_team(name):
    return left_team if name in left_team else right_team


def parse_credits_after(data, start_pos, end_pos):
    """Parse all [10 04 1D] [00 00] [eid BE] [val f32 BE] records in range."""
    credits = []
    pos = start_pos
    while pos < end_pos:
        pos = data.find(CREDIT_HDR, pos, end_pos)
        if pos == -1:
            break
        if pos + 11 > len(data):
            break
        if data[pos+3:pos+5] == b'\x00\x00':
            eid = struct.unpack_from(">H", data, pos+5)[0]
            name = entities_be.get(eid)
            val = struct.unpack_from(">f", data, pos+7)[0]
            if name:
                credits.append({"name": name, "val": val, "pos": pos})
        pos += 1
    return credits


# =========================================================================
# Find kill events
# =========================================================================
kill_events = []
for frame_idx in sorted(frames.keys()):
    data = frames[frame_idx]
    pos = 0
    while True:
        pos = data.find(KILL_HDR, pos)
        if pos == -1:
            break
        if pos + 16 > len(data):
            pos += 1
            continue
        if (data[pos+3:pos+5] != b'\x00\x00' or
            struct.unpack_from(">H", data, pos+5)[0] not in entities_be or
            data[pos+7:pos+11] != b'\xFF\xFF\xFF\xFF' or
            data[pos+11:pos+15] != b'\x3F\x80\x00\x00' or
            data[pos+15] != 0x29):
            pos += 1
            continue

        killer_eid = struct.unpack_from(">H", data, pos+5)[0]
        killer_name = entities_be[killer_eid]
        killer_team = get_team(killer_name)
        enemy_team = right_team if killer_team == left_team else left_team

        # Timestamp
        ts = struct.unpack_from(">f", data, pos - 7)[0] if pos >= 7 else None
        if ts and not (0 < ts < 1800):
            ts = None

        # Parse credits in a 400-byte window after the kill header
        scan_start = pos + 16
        scan_end = min(len(data), pos + 400)
        all_credits = parse_credits_after(data, scan_start, scan_end)

        # Find the kill credit block:
        # Step 1: Find killer's large gold credit (> 50)
        killer_gold_idx = None
        killer_gold = 0
        for ci, c in enumerate(all_credits):
            if c["name"] == killer_name and c["val"] > 50:
                killer_gold_idx = ci
                killer_gold = c["val"]
                break

        # Step 2: Find victim from enemy credit with value (1.0) BEFORE killer gold
        victim = None
        if killer_gold_idx is not None:
            for ci in range(killer_gold_idx):
                c = all_credits[ci]
                if c["name"] in enemy_team and c["val"] == 1.0:
                    # Check if next credit is same player with 0.0
                    if ci + 1 < len(all_credits) and all_credits[ci+1]["name"] == c["name"] and all_credits[ci+1]["val"] == 0.0:
                        victim = c["name"]
                        break

        # Step 3: After killer gold credit, find assist credits
        # Assist = teammate with gold > 2.0 (but < killer gold)
        unique_assisters = set()
        if killer_gold_idx is not None:
            # Credits after the killer gold
            for ci in range(killer_gold_idx + 1, len(all_credits)):
                c = all_credits[ci]
                # Teammate with significant gold
                if (c["name"] != killer_name and
                    c["name"] in killer_team and
                    c["val"] > 2.0 and c["val"] < killer_gold):
                    unique_assisters.add(c["name"])
                # Stop if we see a very small credit from killer (bonus section)
                # or if we see enemy credits
                if c["name"] == killer_name and c["val"] > 2.0 and c["val"] < 50:
                    break  # bonus section started

        # Also try finding victim from death timestamp matching
        victim_from_death = None

        kill_events.append({
            "frame": frame_idx,
            "killer": killer_name,
            "ts": ts,
            "victim_from_credits": victim,
            "assisters": sorted(unique_assisters),
            "killer_gold": killer_gold,
            "all_credits": [f"{c['name']}({c['val']:.1f})" for c in all_credits],
            "pos": pos,
        })
        pos += 16

kill_events.sort(key=lambda e: (e["frame"], e.get("ts") or 0))

# =========================================================================
# Find deaths separately
# =========================================================================
death_events = []
for frame_idx in sorted(frames.keys()):
    data = frames[frame_idx]
    pos = 0
    while True:
        pos = data.find(DEATH_HDR, pos)
        if pos == -1:
            break
        if pos + 13 > len(data):
            pos += 1
            continue
        if data[pos+3:pos+5] != b'\x00\x00' or data[pos+7:pos+9] != b'\x00\x00':
            pos += 1
            continue
        eid = struct.unpack_from(">H", data, pos+5)[0]
        ts = struct.unpack_from(">f", data, pos+9)[0]
        if eid in entities_be and 0 < ts < 1800:
            death_events.append({"victim": entities_be[eid], "ts": ts, "frame": frame_idx})
        pos += 1
death_events.sort(key=lambda e: e["ts"])

# Match kills to deaths by timestamp
used_deaths = set()
for kev in kill_events:
    killer_team = get_team(kev["killer"])
    best_death = None
    best_dt = 999
    best_di = -1
    for di, dev in enumerate(death_events):
        if di in used_deaths:
            continue
        victim_team = get_team(dev["victim"])
        if victim_team != killer_team:
            dt = abs(kev["ts"] - dev["ts"])
            if dt < best_dt and dt < 5.0:
                best_dt = dt
                best_death = dev
                best_di = di
    if best_death:
        kev["victim"] = best_death["victim"]
        kev["death_ts"] = best_death["ts"]
        used_deaths.add(best_di)
    else:
        kev["victim"] = kev.get("victim_from_credits")

# =========================================================================
# Display and validate
# =========================================================================
print("=" * 70)
print("KILL EVENTS (v4)")
print("=" * 70)
print()

kill_counts = Counter()
assist_counts = Counter()

for i, ev in enumerate(kill_events):
    killer = ev["killer"]
    victim = ev.get("victim") or ev.get("victim_from_credits") or "?"
    assisters = ev["assisters"]

    kill_counts[killer] += 1
    for a in assisters:
        assist_counts[a] += 1

    assist_str = ", ".join(assisters) if assisters else "(solo)"
    dt_str = f"dt={abs(ev['ts'] - ev['death_ts']):.2f}s" if ev.get("death_ts") else ""

    print(f"  #{i+1:2d} ts={ev['ts']:8.2f} {killer:8s} -> {victim:8s} "
          f"gold={ev['killer_gold']:.0f} assists=[{assist_str}] {dt_str}")

death_counts = Counter(d["victim"] for d in death_events)

print()
print("=" * 70)
print("KDA VALIDATION (v4)")
print("=" * 70)
print()

print(f"  {'Player':8s} {'K':>3s} {'TK':>3s} {'':12s} {'D':>3s} {'TD':>3s} {'':12s} {'A':>3s} {'TA':>3s}")
total_k = total_d = total_a = 0
for name in ["Phinn", "Yates", "Caine", "Karas", "Petal", "Baron"]:
    ck = kill_counts.get(name, 0)
    cd = death_counts.get(name, 0)
    ca = assist_counts.get(name, 0)
    tk = truth[name]["K"]
    td = truth[name]["D"]
    ta = truth[name]["A"]
    k_ok = "OK" if ck == tk else f"MISS({ck-tk:+d})"
    d_ok = "OK" if cd == td else f"MISS({cd-td:+d})"
    a_ok = "OK" if ca == ta else f"MISS({ca-ta:+d})"
    if ck == tk: total_k += 1
    if cd == td: total_d += 1
    if ca == ta: total_a += 1
    print(f"  {name:8s} {ck:3d} {tk:3d} [{k_ok:>10s}] {cd:3d} {td:3d} [{d_ok:>10s}] {ca:3d} {ta:3d} [{a_ok:>10s}]")

print()
print(f"  ACCURACY: K={total_k}/6  D={total_d}/6  A={total_a}/6")
print(f"  TOTAL: {total_k + total_d + total_a}/18")

# =========================================================================
# Detailed assist breakdown
# =========================================================================
print()
print("=" * 70)
print("ASSIST DETAIL")
print("=" * 70)
print()

for i, ev in enumerate(kill_events):
    killer = ev["killer"]
    killer_team = get_team(killer)
    victim = ev.get("victim") or "?"

    # Show credits in order, marking which ones we count as assists
    print(f"  Kill #{i+1}: {killer} -> {victim}")
    for cred in ev["all_credits"]:
        name, val_s = cred.split("(")
        val = float(val_s.rstrip(")"))
        is_assist = name in ev["assisters"]
        marker = " <<< ASSIST" if is_assist and val > 2.0 else ""
        print(f"    {cred}{marker}")
    print()

# Save
output_path = "D:/Documents/GitHub/VG_REVERSE_ENGINEERING/vg/output/full_kda_v4.json"
output = {
    "version": "full_kda_v4",
    "patterns": {
        "kill_header": "[18 04 1C] [00 00] [killer_eid BE] [FF FF FF FF] [3F 80 00 00] [29 00]",
        "death_record": "[08 04 31] [00 00] [victim_eid BE] [00 00] [ts f32 BE] [00 00 00]",
        "kill_credit": "[10 04 1D] [00 00] [killer_eid BE] [gold f32 BE > 50]",
        "assist_credit": "[10 04 1D] [00 00] [teammate_eid BE] [gold f32 BE > 2, < kill_gold]",
    },
    "accuracy": {"kills": total_k, "deaths": total_d, "assists": total_a, "total": f"{total_k+total_d+total_a}/18"},
    "events": [
        {
            "killer": ev["killer"],
            "victim": ev.get("victim"),
            "assisters": ev["assisters"],
            "killer_gold": ev["killer_gold"],
            "timestamp": ev["ts"],
            "frame": ev["frame"],
        }
        for ev in kill_events
    ],
}
with open(output_path, "w") as f:
    json.dump(output, f, indent=2)
print(f"\nSaved to {output_path}")
print("DONE")
