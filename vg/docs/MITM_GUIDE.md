...
| **영웅 선택** | **100%** | 플레이어 블록 +0xA9 오프셋 uint16 LE |
...
**참고**: 영웅 선택은 VGR 바이너리에서 100% 추출 가능하므로 MITM이 필요하지 않음.
...
| 영웅 선택 | 플레이어 블록 +0xA9 uint16 LE | 100% |
...
**참고**: 영웅 선택은 VGR 바이너리 분석으로 이미 100% 추출 가능 (플레이어 블록 +0xA9 오프셋)
